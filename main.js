var board = new Array();    //box for number
var score = 0;  //score
var has_conflicted = new Array();   //clear mark
var startx = 0; //set start X
var starty = 0; //set start Y
var endx = 0;   //set end X
var endy = 0;   //set end Y
...
//set new game
function new_game() {
    init(); // initial the broad
    //random generate two number
    generate_one_number();
    generate_one_number();
}
...
function init() {
    ...
    for (var i = 0; i < 4; i++) {
        ...
        for (var j =0; j < 4; j++) {
            board[i][j] = 0; // set value for each box
            has_conflicted[i][j] = false; //determine conflicted
        }
    }
    update_board_view();
    score = 0;
    ...
    for (var i = 0; i < 4; i++) {
        for (var j = 0; j < 4; j++) {
            ...
            if (board[i][j] == 0) {
                ...
                number_cell.css('left', get_pos_left(i, j) + cell_side_length / 2); //check number
            } else {
...
function generate_one_number() {
    ...
    if (time <= 50){
        var rand_number = Math.random() < 0.7 ? 2 : 4;
    }else{
        var rand_number = Math.random() < 0.3 ? 2 : 4;
    }
    //place the number
...
// track the movement of the keys
$(document).keydown(function(event) {
    ...
    switch (event.keyCode) {
        case 37: //left
            event.preventDefault(); //Cancel the default action of the click
            if (move_left()) {
...
//listen the start for touch screen
...
document.addEventListener('touchend', function(event) {
    ...
    if (Math.abs(deltax) < 0.15 * document_width && Math.abs(deltay) < 0.15 * document_width) {
        return;
    }
    if ($('#score').text() == success_string) {
        new_game();
            ...
            if (board[i][j] != 0) {
                for (var k = 0; k < j; k++) {
                    if (board[i][k] == 0 && no_block_horizontal(i, k, j, board)) {
                        ...
                        score += board[i][k];
                        update_score(score);
                        has_conflicted[i][k] = true; // prevent continuous erasure
                        break;
                    }
                }
            }
